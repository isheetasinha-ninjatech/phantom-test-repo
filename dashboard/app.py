...
def _wa_json_response(status: int, raw: bytes, content_type: str):
    ...
    return Response(raw, status=status, mimetype=content_type or "application/octet-stream")
...
def whatsapp_panel():
    ...
    return render_template(
        ...
        mode=os.environ.get("PHANTOM_MODE", "").strip().lower() or "slack",
    )
...
@app.route("/api/whatsapp/bind", methods=["POST"])
...
@app.route("/api/whatsapp/unbind", methods=["POST"])
def api_whatsapp_unbind():
    status, raw, ct = _wa_proxy("POST", "/unbind", body={})
    return _wa_json_response(status, raw, ct)


@app.route("/api/whatsapp/bind_method", methods=["POST"])
...
@app.route("/api/whatsapp/retry_bind", methods=["POST"])
def api_whatsapp_retry_bind():
    status, raw, ct = _wa_proxy("POST", "/retry_bind", body={})
    return _wa_json_response(status, raw, ct)


@app.route("/api/whatsapp/bind_now", methods=["POST"])
